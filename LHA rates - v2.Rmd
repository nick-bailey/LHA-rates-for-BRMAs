...
## Introduction
This Rmd creates a single csv file with the weekly Local Housing Allowance (LHA) rates for Broad Rental Market Areas (BRMAs) across the UK from 2012/13 onwards. This version includes Northern Ireland for the first time. 
...
The latest data are for 2024/25. There are no 30th centile rents for any part of Britain for 2012/13 nor for Northern Ireland in 2018. There are a very small number of cases where 30th centile rents are missing, presumably due to a lack of market evidence. 
...
### LHA data sources
At March 2024, the main LHA page was here:  https://www.gov.uk/government/collections/local-housing-allowance-lha-rates. No longer has links to pages for earlier years from 2015/16 onwards as before, only current year. Previously, earlier years (2012-13 to 2014/15) were available through the web archive so possible that more years now found there. In 2012/13, the file only provides LHA rates, not 30th centile rents:
...
### LHA data sources
For the years 2012/13 to 2023/24, data for NI was provided by NIHE in two spreadsheets in response to an FoI request (FOI 358, response 24 April 2024). One contains LHA rates and the other the estimated 30th centile rents. Latest data taken from webpage and stored in 'NI LHA.xlsx'.
...
# brma lha rates
write_csv(df_lha, here("data-output", "lha_rates_2024_v2.csv"))
